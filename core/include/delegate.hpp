// Cross-platform header-only lightweight delegate library
//
...
#ifndef MUL_CORE_DELEGATE_HPP_
#define MUL_CORE_DELEGATE_HPP_
...
template<
	typename R, typename ... Args, template<size_t, size_t, typename, typename ...> class Spec,
	size_t size,
	size_t align>
...
template<typename T, typename R, typename... Args> using closure_decay =
		std::conditional<
...
public:
	...
	template<typename T> explicit pure(T&& func_ptr) noexcept : invoke_ptr_{func_ptr}
	{
...
public:
	...
	explicit inplace_triv(T&& closure) : invoke_ptr_{static_cast<invoke_ptr_t>(
...
public:
	...
	template<
		...
		/*&& std::is_same<decltype(std::declval<T&>()(std::declval<Args>()...)), R>::value>*/>
	...
	template<typename C>
	delegate(C* const object_ptr, R(C::* const method_ptr)(Args...)) noexcept : delegate(
	...
	template<typename C>
	delegate(C* const object_ptr, R(C::* const method_ptr)(Args...) const) noexcept : delegate(
			[object_ptr, method_ptr] (Args&&... args) -> R
			{
				return (object_ptr->*method_ptr)(std::forward<Args>(args)...);
			})
	...
	template<typename C>
	delegate(C& object_ref, R(C::* const method_ptr)(Args...)) noexcept : delegate(
	...
	template<typename C>
	delegate(C& object_ref, R(C::* const method_ptr)(Args...) const) noexcept : delegate(
			[&object_ref, method_ptr] (Args&&... args) -> R
			{
				return (object_ref.*method_ptr)(std::forward<Args>(args)...);
			})
	...
	delegate(R(C::* const method_ptr)(MemArgs...)) noexcept : delegate(
	...
	delegate(R(C::* const method_ptr)(MemArgs...) const) noexcept : delegate(
			[method_ptr] (C* object_ptr, MemArgs... args) -> R
			{
				return (object_ptr->*method_ptr)(std::forward<MemArgs>(args)...);
			})
